...
- В `spacetimedb/src/schema.ts` много таблиц помечено как `public: true`, включая player-scoped и operational данные.
...
- проинвентаризировать текущие `38` таблиц с `public: true` и разнести их по классам `public-by-design`, `player-scoped`, `operational-private`;
...
- `spacetimedb/src/reducers/helpers/all.ts` — около `5100` строк
...
- `spacetimedb/src/reducers/helpers/all.ts` почти наверняка станет ранним кандидатом, если `S2` упрётся в авторизационные изменения и reducer hardening в монолитном helper-слое;
...
- ранние blockers в `spacetimedb/src/reducers/helpers/all.ts` или крупных consumer-поверхностях либо разгружены, либо явно заведены в hotspot track.
