...
- Keep `generate_character_reaction` constrained to display-only runtime output until `player_character_state` and reveal-layer persistence define any future deterministic application path.
- Keep `propose_director_step` (director v1) constrained to VN node-entry auto-enqueue, scenario-level allowed beats, 60-second cooldown plus per-(player, scenario, node) dedupe inside the reducer, and presentation-only `soft_detour` text. Free-form tabletop input is handled separately by `propose_dm_turn`, which returns review-only session-canon proposals from the DM side panel. Reducer-side detour acceptance, fortune/providence cost wiring, state-mutating bounded detours, and director state tables (`director_cursor`, `narrative_pressure`, `beat_status`) remain deferred until the Case01 bridge migration progresses and the director cadence is validated under `VITE_ENABLE_AI_DIRECTOR`.
