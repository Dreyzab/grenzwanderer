...
- Current faction signals already exist as derived `rep_civic`, `rep_underworld`, and `rep_finance` values in `src/features/character/psycheProfile.ts`.
...
`FactionSignals` represent how city blocs and social environments currently interpret the player's methods.

Recommended hidden range per faction:

- `-100..100`

Current temporary mapping in this repo:
...
- The dossier should summarize the strongest current signal and show directionality rather than raw bars in the long-term target UX.
...
| Character dossier psyche tab | `src/features/character/CharacterPanel.tsx` | Faction signal labels, trend, tension summary | Exact `rep_*` values in final UX |
