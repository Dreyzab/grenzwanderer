...
export function MindBoardCanvas({ caseId }: { caseId: string }) {
        ...
        if (pNode) {
          ...
          return {
            ...
            data: {
              ...
              isPlaced:
                pNode.type === "clue"
                  ? pNode.position.x > 250
                  : inNode.data.isPlaced,
            },
          };
        }
        return inNode;
  ...
  const onConnect = useCallback(
    async (params: Connection) => {
      ...
      if (
        !params.source.startsWith("fact-") ||
        !params.target.startsWith("hyp-")
      ) {
        toast.showToast({
      ...
      if (hypData) {
        ...
        setEdges((currentEdges) => {
          const allHypEdges = [...currentEdges, newEdge].filter(
            (e) => e.target === params.target,
          );
          const connectedFactIds = allHypEdges.map((e) =>
            e.source.replace("fact-", ""),
          );

          const isAlreadyValid =
            playerHypothesisMap.get(hypId)?.status === "validated";
    ...
    <div className="w-full h-full relative bg-[#1c1c1f]">
      ...
      >
        <Background
          ...
          style={{ opacity: 0.05 }}
        />
        <Controls className="bg-black/80 border-[#333] fill-white" />
      </ReactFlow>
