...
const isNode = (value: unknown): value is VnNode => {
  ...
  return (
    ...
    (value.sceneGroupId === undefined ||
      typeof value.sceneGroupId === "string") &&
...
export interface VnSnapshotParseIssue {
  path: string;
  message: string;
}

export type VnSnapshotParseResult =
...
export interface VnSnapshotIndex {
  scenariosById: ReadonlyMap<string, VnScenario>;
  nodesById: ReadonlyMap<string, VnNode>;
}

const snapshotIndexCache = new WeakMap<VnSnapshot, VnSnapshotIndex>();

const hasUniqueIds = (entries: ReadonlyArray<{ id: string }>): boolean =>
  new Set(entries.map((entry) => entry.id)).size === entries.length;
...
const indexById = <T extends { id: string }>(
  ...
  for (const entry of entries) {
    if (byId.has(entry.id)) {
      throw new Error(`Duplicate VN ${entityName} id: ${entry.id}`);
    }
...
export const createVnSnapshotIndex = (
  snapshot: VnSnapshot,
): VnSnapshotIndex => {
  ...
  const index: VnSnapshotIndex = {
    scenariosById: indexById(snapshot.scenarios, "scenario"),
    nodesById: indexById(snapshot.nodes, "node"),
  };
...
export const getScenarioById = (
  ...
  createVnSnapshotIndex(snapshot).scenariosById.get(scenarioId) ?? null;

export const getNodeById = (
  snapshot: VnSnapshot,
  nodeId: string,
): VnNode | null =>
  createVnSnapshotIndex(snapshot).nodesById.get(nodeId) ?? null;
